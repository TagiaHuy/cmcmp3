...
const commentService = {
    getCommentsBySongId: async (songId, page = 0, size = 10, signal) => {
        try {
            ...
            if (error.name === 'AbortError') throw error; // Re-throw AbortError so calling hook can ignore it
    ...
    getAdminPendingSongComments: async (page = 0, size = 10, signal) => {
        try {
            const res = await fetch(`${API_BASE_URL}/api/admin/comments/songs/pending?page=${page}&size=${size}`, {
                method: "GET",
                headers: {
                    ...authHeader(),
                    Accept: "application/json",
            ...
            if (error.name === 'AbortError') throw error;
            console.error('Error fetching admin pending song comments:', error);
            throw error;
        }
    },

    getAdminPendingPlaylistComments: async (page = 0, size = 10, signal) => {
        try {
            const res = await fetch(`${API_BASE_URL}/api/admin/comments/playlists/pending?page=${page}&size=${size}`, {
                method: "GET",
                headers: {
                    ...authHeader(),
                    Accept: "application/json",
            ...
            if (error.name === 'AbortError') throw error;
            console.error('Error fetching admin pending playlist comments:', error);
            throw error;
        }
    }
};
