                        ...
                        <Paper key={comment.id} elevation={2} sx={{ p: 2, display: 'flex', flexDirection: 'column', gap: 1 }}>
                            ...
                            <Stack direction="row" alignItems="center" spacing={1}>
                                ...
                                <Typography
                                    ...
                                    onClick={() => navigateToContent(comment.type, comment.song?.id || comment.playlist?.id)}
                                    sx={{ cursor: 'pointer', textDecoration: 'underline' }}
                                >
                                    {comment.song?.title || comment.playlist?.title}
                                </Typography>
                            </Stack>
                            ...
                            <Stack direction="row" spacing={1} sx={{ mt: 2 }}>
                                <Button
                                    ...
                                    onClick={() => handleApprove(comment.type, comment.song?.id || comment.playlist?.id, comment.id)}
                                >
                                    Phê duyệt
                                </Button>
                                <Button
                                    ...
                                    onClick={() => handleReject(comment.type, comment.song?.id || comment.playlist?.id, comment.id)}
                                >
                                    Từ chối
                                </Button>
                            </Stack>
